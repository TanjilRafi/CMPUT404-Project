...
from .models import Author, Post, Inbox, Entry, Comment, Like, Node
...
from .serializers import AuthorSerializer, EntrySerializer, FollowRequestSerializer
...
from .helpers import helper_post_detail, push_post_to_remote, push_post_to_delete, push_post_to_single_author, follow_operation_local_and_remote # all other functions that are not views
...
def author_profile(request, id):
    ...
    friends_count = author.friends.count() # show the real count to other authors too instead of hiding it
...
def authors_list(request):
    """
    Displays a list of all authors *hosted on this node* (local authors only).
    Remote/external authors (from other nodes) are excluded from the UI list to keep the page focused on 
    users who signed up here.

    This fulfills the intent of showing "our" authors in the browser UI, while the API (/api/authors/) 
    still shows everyone for federation.
    """
    # Get the current node's base URL (e.g. http://127.0.0.1:8000 or https://our-app.herokuapp.com)
    current_host = request.scheme + "://" + request.get_host()

    # Normalize: remove any trailing /api/ or / if present, then add /api/ consistently
    # This matches exactly how get_host() in AuthorSerializer normalizes it
    normalized_current = current_host.rstrip("/").rstrip("/api").rstrip("/") + "/api/"

    # Filter authors whose host matches our normalized current host
    # iexact = case-insensitive exact match (safe for http vs https in some edge cases)
    # We use startswith + normalization to catch minor variations
    local_authors = Author.objects.filter(
        host__startswith=normalized_current
...
def home(request):
    ...
    #     Post.objects.filter(visibility=Post.Visibility.DELETED).update(is_deleted=True)     # posts that we sent via remote nodes and are now deleted updated
    #     posts  = (Post.objects.filter(is_deleted=False).filter(visibility=Post.Visibility.PUBLIC).exclude(author_id__exact=current_author.id) | current_author.inbox.posts.filter(is_deleted=False)).distinct().order_by('-created_at')
    # else:
    ...
    # Logged-in users without an Author profile should also go to login
    if not hasattr(request.user, "author"):
        return redirect('login')

    current_author = request.user.author

    Post.objects.filter(visibility=Post.Visibility.DELETED).update(is_deleted=True)
    posts = (
        Post.objects.filter(is_deleted=False)
        .filter(visibility=Post.Visibility.PUBLIC)
        .exclude(author_id__exact=current_author.id)
        | current_author.inbox.posts.filter(is_deleted=False)
    ...
    current_host = request.scheme + "://" + request.get_host()
    current_host = current_host.rstrip("/").rstrip("/api").rstrip("/") + "/api/"
...
def follow_requests(request):
    ...
    if request.method == "GET":
        ...
        parsed = urllib.parse.urlparse(request_author.api_url)
        ...
        if request.POST["action"] == "accept":
            if request_author in user_author.following.all():
                ...
                posts = user_author.posts.filter(is_deleted=False)

                for post in posts:
                    if post not in request_author.inbox.posts.all():
                        request_author.inbox.posts.add(post)
                        if is_remote:
                            push_post_to_single_author(post, request_author)
                ...
                posts = request_author.posts.filter(is_deleted=False)
                ...
                unlisted_posts = user_author.posts.filter(is_deleted=False, visibility__in=[Post.Visibility.UNLISTED, Post.Visibility.PUBLIC])
...
@login_required
@require_http_methods(["POST"])
# Created with the help of Claude AI, Accessed March 23, 2026
def follow(request, id):
    author = request.user.author # actor
    author_to_add = get_object_or_404(Author, id=id) # recipient

    parsed = urllib.parse.urlparse(author_to_add.api_url)
    is_remote = parsed.netloc != request.get_host()

    print(f"[FOLLOW] actor={author.api_url}")
    print(f"[FOLLOW] recipient={author_to_add.api_url}")
    print(f"[FOLLOW] recipient.host={author_to_add.host}")
    print(f"[FOLLOW] is_remote={is_remote}")
    print(f"[FOLLOW] request.get_host()={request.get_host()}")
        ...
        if is_remote:
            try:
                ...
                if response.status_code not in [200, 201]:
                    return redirect('author_profile', id=author_to_add.id)
    ...
    #     print(f"[FOLLOW] follow request payload={serializer.data}")

    #     try:
    #         node = Node.objects.get(url=recipient.host, is_enabled=True)
    #         print(f"[FOLLOW] found node={node.url}, username={node.username}")

    #         inbox_url = f"{recipient.api_url.rstrip('/')}/inbox/"
    #         print(f"[FOLLOW] posting to inbox_url={inbox_url}")
    ...
    #         print(f"[FOLLOW] response body: {response.text}")


    #         if response.status_code not in [200, 201]:
    #             messages.error(request, "Could not send follow request to remote author.")
    #             return redirect('author_profile', id=recipient.id)

    #         if not actor.followers.filter(api_url=recipient.api_url).exists():
    #             actor.following.add(recipient)
    #             posts_to_add = recipient.posts.filter(visibility=Post.Visibility.UNLISTED)
    #             for post in posts_to_add:
    #                 actor.inbox.posts.add(post)
    #         else:
    #             actor.friends.add(recipient)
    #             actor.followers.remove(recipient)
    #             posts_to_add = recipient.posts.filter(visibility__in=[Post.Visibility.UNLISTED, Post.Visibility.FRIENDS])
    #             for post in posts_to_add:
    #                 actor.inbox.posts.add(post)
    #         actor.save()

    #     except Node.DoesNotExist:
    #         messages.error(request, "No enabled node found for this remote author.")
    #     except Exception as e:
    #         print(f"[FOLLOW] ERROR remote follow failed: {e}")
    #         messages.error(request, "Remote node unreachable.")
    # else:
    #     print(f"[FOLLOW] local follow")
    #     if not recipient.inbox.incoming_follow_requests.filter(id=actor.id).exists():
    #         recipient.inbox.incoming_follow_requests.add(actor)
    #         recipient.save()
    #         print(f"[FOLLOW] added actor to recipient inbox follow requests")
    #     else:
    #         print(f"[FOLLOW] actor already in recipient inbox follow requests, skipping")
...
def unfollow(request, id):
    if request.method == "POST":
        ...
        parsed = urllib.parse.urlparse(other.api_url)
        if actor.friends.filter(id=other.id).exists():
                actor.friends.remove(other)
                actor.followers.add(other)

                # Remove other's non-public posts from actor inbox
                posts_to_remove = actor.inbox.posts.filter(author=other).exclude(visibility=Post.Visibility.PUBLIC)
        ...
        # there is no remote unfollow therefore code is the same for both
        # if is_remote:
        #     # Ensure the remote author exists locally
        #     remote_author, _ = Author.objects.get_or_create(
        #         api_url=other.api_url,
        #         defaults={"host": other.host, "displayName": other.displayName},
        #     )

        #     try:
        #         node = Node.objects.get(url=other.host, is_enabled=True) 
        #         unfollow_url = f"{actor.api_url.rstrip('/')}/following/{other.api_url}/" 
        #         response = requests.delete(unfollow_url, auth=(node.username, node.password), timeout=10)
        ...
        #                 actor.friends.remove(other)
        #                 actor.followers.add(other)
        #             elif actor.following.filter(id=other.id).exists():
        #                 actor.following.remove(other)

        #             # Remove posts from inbox as usual
        #             posts_to_remove = actor.inbox.posts.filter(author=other, visibility=Post.Visibility.FRIENDS)
        #             for post in posts_to_remove:
        #                 actor.inbox.posts.remove(post)
        #         else:
        #             messages.error(request, "Could not unfollow remote author.")    # visible on Django admin
        #             return redirect('author_profile', id=other.id)
...
def create_post(request, id):
    ...
    if request.method == 'POST':
        ...
        if title and content:
            # create & save post
            post = Post.objects.create(author=author, title=title, type=type, content=content, contentType=content_type, visibility=visibility)
            push_post_to_remote(post, author) # push to remote nodes after post is saved
            # add to local inboxes based on visibility
            ...
            elif visibility == Post.Visibility.FRIENDS:
...
def edit_post(request, id, post_id):
    ...
    if request.method == 'POST':
        ...
        if type == Post.Type.TEXT:
            ...
            if request.FILES:
                contentType = request.FILES['img'].contentType
                content = base64.b64encode(request.FILES['img'].read()).decode('utf-8')
            elif type == post.type:
        ...
        if title and content:
            if post.title != title or post.type != type or post.visibility != new_visibility or post.content != content:
                post_edited = True
            else:
                post_edited = False
...
def delete_post(request, id, post_id):
    ...
    if request.method == 'POST':
        ...
        recipient_inboxes = post.sent_to.all()
        push_post_to_delete(post, author)   # remote
        
        for inbox in recipient_inboxes: # local
...
def stream(request):
    ...
    # Get posts that arrived in our inbox (this includes remote UNLISTED and FRIENDS posts 
    # that were sent to us because we follow/friend the author)
    inbox_posts = current_author.inbox.posts.filter(is_deleted=False)

    # Combine them and remove duplicates, then sort newest first
    posts = (public_posts | inbox_posts).distinct().order_by('-created_at')
...
def my_friends(request, id=None):
    if id:
        author = get_object_or_404(Author, id=id)
        title = f"{author.displayName}'s Friends"
    else:
        author = request.user.author
        title = "My Friends"
...
#     """
#     Fetches authors from all enabled remote nodes so local users can discover them.
#     """
#     enabled_nodes = Node.objects.filter(is_enabled=True)
#     remote_authors = []

#     for node in enabled_nodes:
#         try:
#             # Construct the /api/authors/ endpoint for the remote node
#             endpoint = f"{node.url.rstrip('/').rstrip('/api')}/api/authors/"
...
#                     raw_host = auth_data.get("host", "").rstrip("/")  # remove trailing slash
#                     # ensure it ends with /api/
#                     if not raw_host.endswith("/api"):
#                         raw_host += "/api"
#                     normalized_host = raw_host + "/" 

#                     author, created = Author.objects.get_or_create(api_url=auth_data.get("id"), defaults={
#                         "url": auth_data.get("web", None),
#                         # "host": auth_data.get("host"),
#                         "host": normalized_host,
#                         "displayName": auth_data.get("displayName", "Unknown"),
#                         "github": auth_data.get("github", ""),
#                         "profileImage": auth_data.get("profileImage", ""),
#                     })
#                     Inbox.objects.get_or_create(author=author)
#                     # auth_data['node_url'] = node.url
#                     # remote_authors.append(auth_data)

#                     # # Fetch remote entries for this author
#                     # entries_url = f"{author.api_url.rstrip('/')}/entries/"
#                     # try:
#                     #     entries_resp = requests.get(entries_url, auth=(node.username, node.password), timeout=5)
#                     #     if entries_resp.status_code == 200:
#                     #         entries_data = entries_resp.json().get("entries", [])
#                     #         for entry in entries_data:
#                     #             # only create if doesn't exist to avoid duplicates
#                     #             Post.objects.get_or_create(
#                     #                 api_url=entry["id"],
#                     #                 defaults={
#                     #                     "title": entry.get("title", ""),
#                     #                     "content": entry.get("content", ""),
#                     #                     "content_type": entry.get("contentType", "text/plain"),
#                     #                     "visibility": entry.get("visibility", Post.Visibility.PUBLIC),
#                     #                     "author": author,
#                     #                     "created_at": entry.get("created_at"),
#                     #                 }
#                     #             )
#                     # except requests.RequestException as e:
#                     #     print(f"Failed to fetch posts for {author.displayName}: {e}")
                    
#         except requests.RequestException as e:
#             print(f"Failed to fetch from {node.url}: {e}")
#             continue

#     current_host = request.scheme + "://" + request.get_host()
#     normalized_current = current_host.rstrip("/").rstrip("/api").rstrip("/") + "/api/"
#     remote_authors = Author.objects.exclude(
#         host__startswith=normalized_current
#     ).order_by('displayName')

#     return render(request, 'authors/discover3.html', {
...
def discover(request):
    """
    Fetches authors from all enabled remote nodes so local users can discover them.
    Also handles Follow by FQID by percent-encoding the FQID before calling the local
    /api/authors/<AUTHOR_ID>/following/<FQID> endpoint.
    """
    # Handle Follow by FQID
    if request.method == "POST":
        fqid = (request.POST.get("fqid") or "").strip().rstrip("/")

        if not fqid:
            messages.error(request, "Please enter a valid FQID.")
            return redirect("discover")

        encoded_fqid = quote(fqid, safe="")
        author_id = request.user.author.id
        following_url = request.build_absolute_uri(
            f"/api/authors/{author_id}/following/{encoded_fqid}"
        )
        ...
        try:
            response = requests.put(
                ...
                headers={
                    ...
                    "Referer": request.build_absolute_uri("/discover/"),
                },
                timeout=10,
            )

            if response.status_code not in [200, 201, 204]:
                print(f"[DISCOVER FQID FOLLOW] PUT failed {response.status_code}: {response.text}")
                messages.error(request, "Problem with foreign node of fqid author")
            else:
                messages.success(request, "Follow request sent successfully.")
                return redirect("discover")

        except requests.RequestException as e:
            print(f"[DISCOVER FQID FOLLOW] ERROR: {e}")
            messages.error(request, "Problem with foreign node of fqid author")
    ...
    for node in enabled_nodes:
        try:
            ...
            if response.status_code == 200:
                ...
                for auth_data in authors_list:
                    raw_host = auth_data.get("host", "").rstrip("/")
                    if not raw_host.endswith("/api"):
                        raw_host += "/api"
                    normalized_host = raw_host + "/"

                    author, created = Author.objects.get_or_create(
                        api_url=auth_data.get("id"),
                        defaults={
                            "url": auth_data.get("web", None),
                            "host": normalized_host,
                            "displayName": auth_data.get("displayName", "Unknown"),
