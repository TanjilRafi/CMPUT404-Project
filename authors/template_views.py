...
from .helpers import helper_post_detail, push_post_to_remote, push_post_to_delete # all other functions that are not views
...
def follow(request, id):
    ...
    parsed = urllib.parse.urlparse(recipient.api_url)
    is_remote = parsed.netloc != request.get_host()

    print(f"[FOLLOW] actor={actor.api_url}")
    print(f"[FOLLOW] recipient={recipient.api_url}")
    print(f"[FOLLOW] recipient.host={recipient.host}")
...
@login_required
@login_required
def my_followers(request, id=None, fqid=None):
    if fqid:
        # Extract UUID from the end of the FQID URL
        author_id = fqid.rstrip('/').split('/')[-1]
...
@login_required
def my_following(request, id=None, fqid=None):
    if fqid:
        author_id = fqid.rstrip('/').split('/')[-1]
        author = get_object_or_404(Author, id=author_id)
