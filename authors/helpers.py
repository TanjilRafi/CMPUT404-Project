...
def _candidate_node_urls(value: str):
    value = _normalize_node_url(value)
    candidates = []

    def add(url: str):
        url = _normalize_node_url(url)
...
def _find_enabled_node_for_host(host: str):
    for candidate in _candidate_node_urls(host):
        node = Node.objects.filter(url=candidate, is_enabled=True).first()
        if node:
            return node

    parsed = urllib.parse.urlparse(host)
    if parsed.netloc:
        return Node.objects.filter(url__icontains=parsed.netloc, is_enabled=True).first()
...
def push_post_to_remote(post, author):
    ...
    for follower in reciever:
        ...
        node = _find_enabled_node_for_host(follower.host)
        if not node:
            print(f"[DEBUG REMOTE PUSH] No enabled Node found for follower host: {follower.host}")
            continue
        ...
        try:
            ...
            print("[DEBUG REMOTE PUSH] PUSHING POST TO REMOTE")
            print("[DEBUG REMOTE PUSH] Inbox URL:", inbox_url)
            print("[DEBUG REMOTE PUSH] Payload:", post_body)
            print("[DEBUG REMOTE PUSH] Response:", response.status_code, response.text)
        except requests.RequestException as e:
            print(f"[DEBUG REMOTE PUSH] Failed to push post: {e}")
            pass
...
#         # created with help of store/update locally
...
# follow request fix 
def get_or_fetch_author(fqid):
    fqid = urllib.parse.unquote_plus(fqid).strip().rstrip("/")
    ...
    parsed = urllib.parse.urlparse(fqid)
    remote_host = f"{parsed.scheme}://{parsed.netloc}"

    node = _find_enabled_node_for_host(remote_host)
    if not node:
        print(f"[DEBUG FQID] No enabled node found for host: {remote_host}")
        return None
    ...
    try:
        ...
        # handle both spec-style "web" and older "url"
        web_url = data.get("web") or data.get("url") or fqid.replace("/api/", "/").rstrip("/")

        author, _ = Author.objects.update_or_create(
            api_url=fqid,
            defaults={
                "displayName": data.get("displayName", ""),
                "host": data.get("host") or (remote_host + "/api/"),
                "url": web_url,
        ...
        print(f"[DEBUG FQID] Fetch failed for {fqid}: {e}")
        return None

def push_post_to_delete(post, author):
    """
    Push a deleted post update to the inboxes of all remote followers and friends.
    """
    if post.visibility == Post.Visibility.PUBLIC:
        reciever = list(Author.objects.exclude(host=author.host))
    elif post.visibility == Post.Visibility.UNLISTED:
    ...
    for follower in reciever:
        ...
        node = _find_enabled_node_for_host(follower.host)
        if not node:
            print(f"[DEBUG DELETE PUSH] No enabled Node found for follower host: {follower.host}")
            continue
        ...
        try:
            ...
            print("[DEBUG DELETE PUSH] PUSHING POST DELETE TO REMOTE")
            print("[DEBUG DELETE PUSH] Inbox URL:", inbox_url)
            print("[DEBUG DELETE PUSH] Payload:", post_body)
            print("[DEBUG DELETE PUSH] Response:", response.status_code, response.text)
        except requests.RequestException as e:
            print(f"[DEBUG DELETE PUSH] Failed to push deleted post: {e}")
            pass
...
# fixed follow request push and single post push 
# def push_post_to_single_author(post, author):
#     node = _find_enabled_node_for_host(author.host)

#     if not node:
#         print(f"[DEBUG SINGLE PUSH] No enabled Node found for host: {author.host}")
#         return False

#     post_body = EntrySerializer(post).data
#     inbox_url = author.api_url.rstrip("/") + "/inbox"
...
#             timeout=5,
#         )
#         print("[DEBUG SINGLE PUSH] PUSHING POST TO SINGLE AUTHOR")
#         print("[DEBUG SINGLE PUSH] Inbox URL:", inbox_url)
#         print("[DEBUG SINGLE PUSH] Payload:", post_body)
#         print("[DEBUG SINGLE PUSH] Response:", response.status_code, response.text)
#         return response.ok
#     except requests.RequestException as e:
#         print(f"[DEBUG SINGLE PUSH] Failed to push post: {e}")
#         return False
...
def push_post_to_single_author(post, author):
    ...
    try:
        ...
        print(f"[DEBUG SINGLE PUSH] Failed: {e}")
        return False
