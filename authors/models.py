...
class Post(models.Model):
    ...
    contentType = models.CharField(max_length=18, null=True, db_column="content_type")
    def __init__(self, *args, **kwargs):
        if "content_type" in kwargs and "contentType" not in kwargs:
    ...
    def setType(self):
        ...
        elif self.contentType == "text/markdown":
...
class Inbox(models.Model):
    ...
    posts = models.ManyToManyField(Post, related_name="sent_to")        # removed symmetrical = True due to warnings during migrations. related_name has no effect with a symmetrical relationship
...
class Comment(models.Model):
    ...
    contentType = models.CharField(max_length=50, default="text/plain", db_column="content_type")
    def __init__(self, *args, **kwargs):
        if "content_type" in kwargs and "contentType" not in kwargs:
            kwargs["contentType"] = kwargs.pop("content_type")
        super().__init__(*args, **kwargs)
...
class Like(models.Model):
    ...
    #entry = models.ForeignKey('Entry', on_delete=models.CASCADE, null=True, blank=True, related_name='likes')
...
class Entry(models.Model):
    ...
    uuid = models.UUIDField(default=django_uuid.uuid4, editable=False, unique=True)
...
class Node(models.Model):
    ...
    def save(self, *args, **kwargs):
        # authomatically create a user instance
        if not self.user:
            self.user, _ = User.objects.get_or_create(username=self.username, password=self.password)
