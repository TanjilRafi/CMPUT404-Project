...
def api_inbox(request, id):
    ...
    if type_str == "entry":
        try:
            ...
            elif post.visibility == Post.Visibility.UNLISTED and (author.following.filter(api_url=actor.api_url).exists() or author.friends.filter(api_url=actor.api_url).exists()):
                author.inbox.posts.add(post)
            elif post.visibility == Post.Visibility.FRIENDS and author.friends.filter(api_url=actor.api_url).exists():
                author.inbox.posts.add(post)
