...
def api_entries(request, id):
    ...
    if request.method == "GET":
        ...
        # TODO: reformat to fit EntriesSerializer
        # data = {
        #     "type": "entries",
        #     "items": [
        #         {
        ...
        # Using stored api_url / url instead of re-constructing from host + id parts.
        # Re-constructing risks subtle mismatches (trailing slash, /api/ prefix) between nodes; 
        # using the pre-computed, canonical value guarantees consistency (US 08.16).
        data = {
            "type": "entries",
            "page_number": page,
            "size": size,
            ...
            "items": [
                {
                    "type": "entry",
                    "id": p.api_url,        # canonical full API URL, stored at creation time
                    "web": p.url,           # canonical browser URL, stored at creation time
                    "title": p.title,
                    "content": p.content,
                    ...
                    "author": {
                        "type": "author",
                        "id": author.api_url,   # full API URL, not a bare UUID
                        "host": author.host,
                    ...
                    "published": p.created_at.isoformat(),
                }
                for p in posts
            ],
        }
...
def api_entry_detail(request, id, post_id):
    ...
    if request.method == "GET":
        ...
        # Canonical URLs are stored on the model at save() time (US 08.16).
        # Using them here prevents URL reconstruction bugs when host strings differ between nodes (e.g. trailing slash, /api/ suffix presence).
        data = {
            "type": "entry",
            "id": post.api_url,
            "web": post.url,
            ...
            "author": {
                ...
                "github": post.author.github or "",
                "profileImage": post.author.profileImage or "",
                "web": post.author.url,
            },
            "published": post.created_at.isoformat(),
        }
        # return JsonResponse(EntrySerializer(post).data)
        return JsonResponse(data)
