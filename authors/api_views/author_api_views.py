...
from authors.serializers import AuthorSerializer, EntrySerializer, AuthorsSerializer
...
@extend_schema(
    ...
    **Why not use UI**: This is the machine-readable/API version; use the browser authors list for human viewing.
    Example: curl http://127.0.0.1:8000/api/authors?page=1&size=5""",
    parameters=[
        OpenApiParameter(
            name="page",
            type=int,
    ...
    responses={
        ...
        400: OpenApiResponse(
            description="Invalid `page` or `size` query parameter.",
            examples=[
                OpenApiExample(
                    name="Invalid page example",
                    value={"error": "Invalid page or size"}
                )
            ]
        )
    },
...
def api_authors_list(request):
    ...
    serializer = AuthorsSerializer({"authors": paginated})
    response_data = serializer.data

    # FETCH REMOTE AUTHORS

    active_nodes = Node.objects.filter(is_enabled=True)
    remote_authors = []

    for node in active_nodes:
        remote_url = f"{node.url.rstrip('/')}/api/authors/"
        
        try:
            params = {'page': page, 'size': size}
            auth_credentials = (node.username, node.password)
            resp = requests.get(remote_url, auth=auth_credentials, params=params, timeout=3)
            
            if resp.status_code == 200:
                data = resp.json()
                if "items" in data:
                    remote_authors.extend(data["items"])
                elif "authors" in data:
                    remote_authors.extend(data["authors"])
                    
        except requests.exceptions.RequestException:
            continue
        
    # Append the remote authors to the local ones
    if "authors" in response_data:
        response_data["authors"].extend(remote_authors)
    elif "items" in response_data:
        response_data["items"].extend(remote_authors)
...
@extend_schema(
    ...
    Example: curl -X GET http://127.0.0.1:8000/api/authors/c7d84ea2-27bb-44ba-8e9c-07bc149a88ee/
    """,
    responses={
        200: OpenApiResponse(
...
@extend_schema(
    ...
    When to use PUT: When an author wants o update their displayName, github, or profileImage via the API.
    ...
    Example: 
    curl -X PUT http://127.0.0.1:8000/api/authors/c7d84ea2-27bb-44ba-8e9c-07bc149a88ee/ \
     -H "Content-Type: application/json" \
     -d '{
           "displayName": "Mysterio",
           "github": "http://github.com/mysterio",
...
@extend_schema(
    methods=["GET"],
    summary="Get a single author by fqid",
    description="""GET: Returns a single author's public profile as JSON.

    When to use GET: When you need to look up a specific author's identity info by fqid.

    How to use GET: Send a GET request with the author's fqid in the URL. Authentication required, allows node authentication.

    Pagination: Not applicable — returns a single object.

    Example: curl -X GET http://127.0.0.1:8000/api/authors/http://127.0.0.1:8000/api/authors/c7d84ea2-27bb-44ba-8e9c-07bc149a88ee/
    """,
    responses={
        200: OpenApiResponse(
            response=AuthorSerializer,
...
@authentication_classes([NodeBasicAuthentication, SessionAuthentication, BasicAuthentication])
@permission_classes([IsAuthenticated])
def api_author_profile_fqid(request, fqid):
    """
    GET /api/authors/{fqid}/ — get author
    """
    fqid_clean = urllib.parse.unquote_plus(fqid.rstrip("/")).rstrip("/")
    author = Author.objects.filter(api_url=fqid_clean).first()
    if author:
        if request.method == "GET":
            serializer = AuthorSerializer(author)
            return JsonResponse(serializer.data)
            
    # 3. If not found locally, proxy the request to the remote node
    if request.method == "GET":
        parsed_url = urlparse(fqid_clean)
        remote_host = parsed_url.netloc # Gets the "team-other.herokuapp.com" part
        
        node = Node.objects.filter(url__icontains=remote_host, is_enabled=True).first()
        
        if not node:
            return JsonResponse({"detail": "Author not local, and no matching remote node found."}, status=404)
            
        try:
            auth_credentials = (node.username, node.password) 
            
            response = requests.get(fqid_clean, auth=auth_credentials, timeout=5)
            response.raise_for_status() 
            return JsonResponse(response.json())
            
        except requests.exceptions.RequestException as e:
            return JsonResponse({"detail": f"Failed to fetch from remote node: {str(e)}"}, status=502)