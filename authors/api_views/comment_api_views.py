...
from authors.helpers import _comment_to_api, _comment_id_from_fqid, push_comment_to_remote
...
@permission_classes([IsAuthenticated])
def api_entry_comments_fqid(request, fqid):
...
class ApiAuthorCommented(APIView):
    ...
    def get(self, request, author_id):
        """
        GET  /api/authors/{author_id}/commented/ — list comments (Local + Remote)
        """
        ...
        local_qs = Comment.objects.filter(author=author).select_related("post", "author")
        all_comments = [_comment_to_api(c) for c in local_qs]

        # Fetch Remote Comments from enabled nodes
        enabled_nodes = Node.objects.filter(is_enabled=True)
        for node in enabled_nodes:
            node_base = node.url.rstrip('/')
            if not node_base.endswith('api'):
                node_base += '/api'
                
            endpoint = f"{node_base}/authors/{author_id}/commented/"
            ...
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    remote_items = data.get("src", data.get("items", []))
                    all_comments.extend(remote_items)
        ...
        for c in all_comments:
            c_id = c.get("id")
            if c_id and c_id not in unique_comments:
                unique_comments[c_id] = c
    ...
    def post(self, request, author_id):
        """
        POST /api/authors/{author_id}/commented/ — create comment
        """
        author = get_object_or_404(Author, id=author_id)
...
def post_comment(request, author_id, post_id):
    ...
    if content:
        comment = Comment.objects.create(post=post, author=request.user.author, content=content)
        push_comment_to_remote(comment, request.user.author, post)
