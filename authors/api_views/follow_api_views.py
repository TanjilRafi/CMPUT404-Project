    ...
    if request.method == "GET":
        ...
        if urllib.parse.urlparse(api_url).netloc != request.get_host():
            ...
            data = {"type":"follow", 
                    ...
                    "object": AuthorSerializer(author_to_add).data,
                    }
            ...
            if response.status_code!=200:
                ...
                author.following.add(author_to_add)
        else:
            # local
            if not author_to_add.inbox.incoming_follow_requests.filter(id=author.id).exists() and not author.following.filter(id=author_to_add.id).exists():
