...
from authors.serializers import AuthorSerializer, EntrySerializer, LikeSerializer, LikesSerializer
...
from authors.helpers import serialize_like, push_like_to_remote, push_comment_like_to_remote
...
def like_post(request, author_id, post_id):
    ...
    if not created:
        Like.objects.filter(id=str(like.id)).delete()
    else:
        # push to remote only when creating a new like, not when unliking
        push_like_to_remote(like, request.user.author, post)
...
def like_comment(request, author_id, post_id, comment_id):
    ...
    if not created:
        ...
        push_comment_like_to_remote(like, request.user.author, comment)
...
def api_entry_likes(request, id, post_id):
    ...
    entry = get_object_or_404(Post, id=post_id)
    
    if entry.visibility == Post.Visibility.DELETED:
        return Response({"error": "Entry not found"}, status=404)
    
    if entry.visibility == Post.Visibility.FRIENDS:
        ...
        try:
            ...
            if viewer_author != entry.author and viewer_author not in entry.author.friends.all():
                return Response({"detail": "Not authorized to view friends-only likes"}, status=403)
        except Exception:
            return Response({"detail": "Not authorized"}, status=403)
        
    likes_qs = Like.objects.filter(post=entry, comment=None).order_by('-published')
    ...
    start = (page - 1) * size
    paginated_likes = likes_qs[start:start + size]
    serializer = LikeSerializer(paginated_likes, many=True)

    data = {
        "type": "likes",
        "id": f"{entry.api_url}/likes", 
        "page_number": page,
        "size": size,
...
@permission_classes([IsAuthenticated])
@permission_classes([])
def api_author_liked(request, id):
    """GET list of things liked by this author, or single like by serial"""
    page_number = int(request.GET.get('page', 1))
    size = int(request.GET.get('size', 10))
    author = get_object_or_404(Author, id=id) 
    return JsonResponse(LikesSerializer(author, context={"size":size, "page_number":page_number}).data)
...
def api_author_liked_fqid(request, fqid):
    ...
    page_number = int(request.GET.get('page', 1))
    size = int(request.GET.get('size', 10))
    author = get_object_or_404(Author, api_url=fqid) 
    return JsonResponse(LikesSerializer(author, context={"size":size, "page_number":page_number}).data)
...
@authentication_classes([SessionAuthentication, BasicAuthentication])
@permission_classes([IsAuthenticated])
def api_single_like_fqid(request, fqid):
    """GET a single like object by its full FQID"""
    api_url = urllib.parse.unquote_plus(fqid.rstrip("/")).rstrip("/")
    like = get_object_or_404(Like, api_url=api_url)
    return JsonResponse(LikeSerializer(like).data)

@extend_schema(
    methods=["GET"],
    summary="Get a single like by serials",
    description=
    """
    Retrieves a single like object by the author's and like's serials.
    ...
    parameters=[
        OpenApiExample(
            name="author_serial",
            value="8569f79b-7607-48ce-9570-74c528090ac5",
            description="serial"
        ),
        OpenApiExample(
            name="like_serial",
            value=" 29978a2e-e3fd-43f6-9706-25e8fa9bb9ad ",
            description="serial"
        )
    ],
    responses={
...
@authentication_classes([NodeBasicAuthentication, SessionAuthentication, BasicAuthentication])
@permission_classes([IsAuthenticated])
def api_single_like(request, author_id, like_id):
    """GET a single like object by author and like serials"""
    author = get_object_or_404(Author, id=author_id)
...
@extend_schema(
    ...
    description="""Retrieves a paginated list of likes on the specified comment.
    Identifies the comment by its FQID. Inherits visibility rules from the parent post.
    """,
    parameters=[
        OpenApiParameter(name="author_id", description="UUID of the post author", required=True, type=OpenApiTypes.STR, location=OpenApiParameter.PATH),
        OpenApiParameter(name="post_id", description="UUID of the post", required=True, type=OpenApiTypes.STR, location=OpenApiParameter.PATH),
        OpenApiParameter(name="comment_fqid", description="FQID of the comment (URL-encoded)", required=True, type=OpenApiTypes.STR, location=OpenApiParameter.PATH),
        OpenApiParameter(name="page", description="Page number", required=False, type=OpenApiTypes.INT, location=OpenApiParameter.QUERY),
        OpenApiParameter(name="size", description="Items per page", required=False, type=OpenApiTypes.INT, location=OpenApiParameter.QUERY),
    ]
)
@api_view(["GET"])
@authentication_classes([NodeBasicAuthentication, SessionAuthentication, BasicAuthentication])
@permission_classes([IsAuthenticated])
def api_comment_likes(request, author_id, post_id, comment_fqid):
    """GET list of likes on a specific comment"""

    api_url = urllib.parse.unquote_plus(comment_fqid.rstrip("/")).rstrip("/")
    comment_uuid = api_url.split("/")[-1]
    
    comment = Comment.objects.filter(Q(api_url=api_url) | Q(id__icontains=comment_uuid)).first()
    if not comment:
        return Response({"detail": "Comment not found"}, status=404)
        
    post = comment.post
    if getattr(post, 'is_deleted', False) or post.visibility == 'DELETED':
    ...
    if post.visibility == 'FRIENDS':
        ...
        try:
            viewer_author = request.user.author
            if viewer_author != post.author and viewer_author not in post.author.followers.all():
                return Response({"detail": "Not authorized to view friends-only likes"}, status=403)
        except Exception:
            return Response({"detail": "Not authorized"}, status=403)
            
    likes_qs = Like.objects.filter(comment=comment).order_by('-published')
