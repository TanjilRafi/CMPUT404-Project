...
from authors.serializers import AuthorSerializer, EntrySerializer, LikeSerializer
...
def api_entry_likes(request, id, post_id):
    ...
    entry = Entry.objects.filter(Q(uuid=post_id) | Q(id__icontains=str(post_id))).first()
    if not entry:
    ...
    if entry.visibility == 'FRIENDS':
        ...
        try:
            ...
            if viewer_author != entry.author and viewer_author not in entry.author.followers.all():
                return Response({"detail": "Not authorized to view friends-only likes"}, status=403)
        except Exception:
            return Response({"detail": "Not authorized"}, status=403)
        
    likes_qs = Like.objects.filter(entry=entry).order_by('-published')
